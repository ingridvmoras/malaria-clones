    ...
    with PdfPages(pdf_path) as pdf:
        for unique_id in sample[id].unique():
            ...
            for i, method in enumerate(methods):
                ...
                plot_data = sample_id.drop_duplicates(subset=['Timepoint', id], keep='first').copy()
            ...
            plt.suptitle(f'{unique_id}', fontsize=16, fontweight='bold')
...
def venndiagram(df_tph,df_lm,df_s1,id,save_path):
     # Extract sets of (Timepoint, id) for each method
        tph_set = set(df_tph[df_tph['peak'] == True][['Timepoint', id]].apply(tuple, axis=1))
        lm_set = set(df_lm[df_lm['peak'] == True][['Timepoint', id]].apply(tuple, axis=1))
        s1_set = set(df_s1[df_s1['peak'] == True][['Timepoint', id]].apply(tuple, axis=1))
       
    
        plt.figure(figsize=(10, 7))
        venn3([tph_set, lm_set, s1_set], ('TPH', 'LM', 'S1'),
              set_colors=("#999999", "#E69F00", "#56B4E9"), alpha=0.7)
        venn3_circles([tph_set, lm_set, s1_set], 
              linestyle="dashed", linewidth=2)
        plt.savefig(f'{save_path}/venn_diagram.png')
        plt.close()
