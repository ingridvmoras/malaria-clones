...
def peaks(df,id,output_dir):
    df = df[df['peak'] == True]
    df_grouped = df.groupby([id, 'Timepoint'])['Method'].apply(lambda x: ', '.join(x)).reset_index()
    df_grouped = df_grouped.rename(columns={'Method': 'Methods'})
    df = df.merge(df_grouped, on=['Kid', 'Timepoint'])
    df = df.drop_duplicates(subset=[id, 'Timepoint'])
    df.to_csv(f'{output_dir}/peaks.csv', index=False) 